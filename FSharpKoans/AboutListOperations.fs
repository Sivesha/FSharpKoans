...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            List.fold (fun sum (x)->sum+x) initialState xs // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            List.fold (fun mul (x)->mul*x) initialState xs  // write a function to multiply the elements of a list
